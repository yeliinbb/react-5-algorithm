...
주어진 문자열 allowed와 문자열 배열 words가 있습니다. allowed 문자열은 서로 다른 문자들로 이루어져 있습니다. 각 words 배열의 문자열이 allowed에 있는 문자들만 포함하고 있을 때, 
...
function countConsistentStrings(allowed, words) {
  // allowed 문자열 배열로 쪼개기
  const allowedArr = allowed.split("").sort();
  console.log("---", allowedArr);
  let count = 0;
  // words 배열 요소를 하나씩 순회하면서 allowedArr 배열 요소가 있는지 확인
