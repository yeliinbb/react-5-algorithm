...
function minimumNumberGame(nums) {
  ...
  // 나중에 출력을 할 빈 배열 선언
  let result = [];
  // nums while loop 사용해서 반복, break point nums에 숫자가 하나도 남아있지 않은 경우
  while (nums.length > 0) {
    // 앨리스가 최소값을 먼저 가져감
    const aliceMinIndex = findIndex(nums);
    const aliceMinValue = nums.splice(aliceMinIndex, 1)[0];
    // nums에 숫자가 남아있는지 확인하고
    // 만약에 남아있다면 밥도 최소값을 가져감
    // 밥이 가져간 값을 먼저 출력할 배열에 넣어줌
    if (nums.length > 0) {
      const bobMinIndex = findIndex(nums);
      const bobMinValue = nums.splice(bobMinIndex, 1)[0];
      result.push(bobMinValue);
    }
    // 이후에 앨리스가 가져간 값을 추가해줌
